...
import type { Ypodeigma1TableBRow, Ypodeigma1TableCRow } from './types';

type SupportedRow = Ypodeigma1TableBRow | Ypodeigma1TableCRow;
...
function getAnalysisLevel(row: SupportedRow) {
  return row.analysisLevel ?? Math.min(getRowDepth(row.code) + 1, ANALYSIS_COLUMNS.length);
}

const Ypodeigma1Section1BTable: FC<Props> = ({
  tableCode,
  title,
  rows,
  isEditable,
      ...
      <table className="w-full table-fixed border-collapse text-[9px] text-slate-800">
        <colgroup>
          ...
          {ANALYSIS_COLUMNS.map((column) => (
            <col key={`analysis-col-${tableCode}-${column}`} className="w-7" />
          ))}
          <col className="w-64" />
          <col className="w-28" />
        </colgroup>
        ...
        <tbody>
          {rows.map((row) => {
            ...
            const calculatedAmount = calculateRowAmount(row, rows);
            ...
            return (
              <tr key={row.id} className={leafRow ? 'bg-white' : 'bg-sky-50'}>
                ...
                {ANALYSIS_COLUMNS.map((column) => (
                  <td
                    key={`${row.id}-${tableCode}-analysis-${column}`}
...
export default Ypodeigma1Section1BTable;
